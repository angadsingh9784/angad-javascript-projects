const text = [
  ...
  const test=document.querySelector('#inputText');
  ...
  butn.addEventListener('click',(e)=>{
    ...
    const val=parseInt(test.value);
    // console.log(typeof(val))
    const randomNum=Math.floor(Math.random()*10);
    // console.log(randomNum)
    if(isNaN(val)||val<0||val>9){
        paras.innerHTML=`${text[randomNum]}`
    }else{
     let tempText=text.slice(0,val);
     tempText=tempText.map((elem)=>{
      return `<p class="result">${elem}</p><br>`;
    }).join(" ");
      paras.innerHTML=tempText;
